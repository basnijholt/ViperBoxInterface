...
from ctypes import c_int, POINTER, c_bool, c_char, c_char_p, c_int16, c_size_t, c_uint8, c_void_p, sizeof
...
if _platform.startswith('cygwin'):
    import subprocess
    _lib_path = subprocess.check_output(["cygpath", "-w", _lib_path]).decode("utf-8").strip()
...
def setLogLevel(level: LogLevel) -> None:
    ...
    if not isinstance(level, LogLevel):
        raise NeuraviperAPIError(42, "Debug level must be set using DebugLevel enumeration!")
...
@_wrap_function("transferSPI", 
    NVP_ErrorCode, [DeviceHandle, c_uint8, POINTER(c_uint8), POINTER(c_uint8), c_size_t])
...
@_wrap_function("writeSPI", NVP_ErrorCode, [DeviceHandle, c_uint8, POINTER(c_uint8), c_size_t])
...
@_wrap_function(
    "readElectrodeData", 
    NVP_ErrorCode,
    [DeviceHandle, c_uint8, POINTER(PacketInfo), POINTER(c_int16), c_int, c_int, POINTER(c_int)])
def readElectrodeData(handle: DeviceHandle, probe: int, packet_count: int) -> list[Packet]:
    #Read electrode data
    ...
    info_arr = (PacketInfo * packet_count)()    #Ctypes way of defining an array of packet_count times a PacketInfo element; (): empty
    data_arr = (c_int16 * (packet_count * channel_count))()
    
    __assertnvperror(_c_fn(handle, probe, info_arr, data_arr, channel_count, packet_count, packets_read))
...
@_wrap_function("readMezzanineHardwareID", NVP_ErrorCode, [DeviceHandle, c_uint8, POINTER(HardwareID)])
def readMezzanineHardwareID(handle: DeviceHandle, probe: int) -> HardwareID:
    hwid = HardwareID()
    __assertnvperror(_c_fn(handle, probe, hwid))
    return hwid

@_wrap_function("readProbeHardwareID", NVP_ErrorCode, [DeviceHandle, c_uint8, POINTER(HardwareID)])
...
@_wrap_function("streamOpenFile", NVP_ErrorCode, [c_char_p, POINTER(StreamHandle), c_uint8])
...
def streamReadData(handle: StreamHandle, packet_count: int) -> list[Packet]:
    ...
    error_code = _c_fn(handle, info_arr, data_arr, channel_count, packet_count, packets_read)
    # STREAM_EOF = 26
...
@_wrap_function("bistStopPRBS", NVP_ErrorCode, [DeviceHandle, POINTER(c_int), POINTER(c_int)])
...
@_wrap_function("bistReadPRBS", NVP_ErrorCode, [DeviceHandle, POINTER(c_int), POINTER(c_int)])
