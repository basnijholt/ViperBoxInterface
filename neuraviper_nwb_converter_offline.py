...
layout = [
    ...
    [
        name("INSTITUTION"),
        sg.Input(
            default_text="Netherlands Institute for Neuroscience",
            key="-INSTITUTION-",
            s=45,
        ),
    ],
    [
        name("LAB"),
        sg.Input(default_text="Vision and Cognition Group", key="-LAB-", s=45),
    ],
    [
        name("EXPERIMENTER"),
        sg.Input(default_text="JA Westerberg", key="-EXPERIMENTER-", s=45),
    ],
    [name("SUBJECT"), sg.Input(default_text="toucan", key="-SUBJECT-", s=45)],
    [name("SESSION ID"), sg.Input(default_text="000", key="-SESSION ID-", s=45)],
    [
        name("EXPERIMENT"),
        sg.Input(default_text="conversion_demo", key="-EXPERIMENT-", s=45),
    ],
    [sg.Button("RUN", size=(63, 1), key="-RECORD-", metadata=False)],
]

window = sg.Window(title="ViperBox NWB Stream Converter", layout=layout)
...
while True:
    ...
    if event == "-FOLDER-":
        ...
        fnames = [
            f
            for f in file_list
            if os.path.isfile(os.path.join(folder, f)) and f.lower().endswith((".bin"))
        ]
    ...
    while window["-RECORD-"].metadata:
        if os.path.exists(
            os.path.join(values["-FOLDER-"], values["-FILE LIST-"][0][:-3] + "nwb")
        ):
            window["-RECORD-"].metadata = not window["-RECORD-"].metadata
        ...
        if (
            ~window["-TOGGLE PROBE 0-"].metadata
            & window["-TOGGLE PROBE 1-"].metadata
            & ~window["-TOGGLE PROBE 2-"].metadata
            & ~window["-TOGGLE PROBE 3-"].metadata
        ):
            window["-RECORD-"].metadata = not window["-RECORD-"].metadata
            break
        ...
        viperbox0_ElectrodeColumn = nwbfile.add_electrode_column(
            name="label", description="label of viperbox0 electrodes"
        )
        ...
        all_table_region = nwbfile.create_electrode_table_region(
            region=list(range(electrode_counter)), description="all electrodes"
        )
        ...
        if window["-TOGGLE PROBE 1-"].metadata:
            ...
            if not window["-TOGGLE PROBE 0-"].metadata:
                ...
                current_time = np.asarray(
                    [stream[i].timestamp for i in range(np.size(stream))],
                    dtype="uint32",
                )
                current_time = current_time / 1000.0
        ...
        if window["-TOGGLE PROBE 2-"].metadata:
            ...
            if (
                not window["-TOGGLE PROBE 0-"].metadata
                and not window["-TOGGLE PROBE 1-"].metadata
            ):
                current_data = np.asarray(
                    [stream[i].data for i in range(np.size(stream))], dtype="uint16"
                )
        ...
        if window["-TOGGLE PROBE 3-"].metadata:
            ...
            if (
                not window["-TOGGLE PROBE 0-"].metadata
                and not window["-TOGGLE PROBE 1-"].metadata
                and not window["-TOGGLE PROBE 2-"].metadata
            ):
                current_data = np.asarray(
                    [stream[i].data for i in range(np.size(stream))], dtype="uint16"
                )
